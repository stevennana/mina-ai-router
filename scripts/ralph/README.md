# Mina AI Router Ralph Loop

This package adds a Ralph-style **task-promotion loop** to Mina AI Router.
...
This loop makes that concrete for Mina AI Router by treating the task docs as executable
...
- `scripts/ralph/ensure-e2e-port-free.sh`: clears a configured browser-test port before E2E-capable verification commands
...
Before Playwright-backed verification, the harness can clear a browser-test port.
Mina AI Router's current default verification relies on Node smoke scripts rather
than a dedicated Playwright web server, so most 0.2 tasks should not need port cleanup.
...
- HTTP smoke verification writes command output into Ralph cycle artifacts.
- manual operator runs can use `mair server start --port 3333` and inspect server state with `mair server status`, `mair health`, and the browser console.
...
- Port cleanup is executed automatically only by the evaluator path for Playwright-bearing commands. Manual local runs do not get that cleanup for free.
- `ensure-e2e-port-free.sh` is intentionally aggressive and may terminate unrelated processes bound to the configured port.
