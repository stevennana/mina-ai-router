...
function commandNeedsE2ePortCleanup(command) {
  return /playwright|test:e2e/.test(command);
}
...
function ensureE2ePortIsFree(command) {
  ...
  const port = process.env.RALPH_E2E_PORT ?? "3334";
...
function buildCommandEnvironment(command) {
  ...
  const serverLogPath = artifactDir
    ? path.join(process.cwd(), artifactDir, `${slugifyCommand(command)}-server.log`)
    : path.join(STATE_DIR, `${slugifyCommand(command)}-server.log`);

  return {
    serverLogPath,
    value: {
      ...process.env,
      MINA_ROUTER_RALPH_SERVER_LOG: serverLogPath,
    },
  };
}
...
function runShellCommand(command) {
  ...
  if (preflight && !preflight.ok) {
    return {
      ...
      preflight,
      ...(commandEnvironment
        ? { server_log_path: commandEnvironment.serverLogPath }
        : {}),
    };
  }
